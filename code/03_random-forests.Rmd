...
Why are Random Forests a good choice:
  - Easy to use & work with
  - Easy to program
  - Fast
  - Good performance
  - Not necessarily a lot of tuning
...
custom_rf <- function(data, label, ...) {
  set.seed(42)
  randomForest(label ~ ., data = data, importance = TRUE, ...)
}
lapply(ntrees, function(x) custom_rf(train_wine[,-quality_ind], train_wine$quality, ntree = x))
...
To do:
  - Review how splits are made --> Gini Impurity